#!/usr/bin/env python3
"""Shared PreToolUse hook for Bash: deny `rm -f` with an explanation.

Project permission rules already block these commands where supported, but a bare
denial teaches the agent nothing. This hook gives Claude Code and Codex the same explicit
reason, so the agent understands that the fix is to drop the `f` flag — not to
retry, escalate, or work around the block.
"""
import json
import re
import sys
