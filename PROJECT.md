...
- First run is complete. The current GitHub workflow checks changes; deployment will be added as part of the launch implementation. Half-Life: Continuum Edition portfolio v1 is accepted; extra gallery images are v2. BeamVM's approved write-up has resumed media/metadata work now that the user supplied its screenshot guide. FantasyBoy's article/gallery/dates are in review. Both drafts should show known languages, tags, and development spans, with pending labels for unknown fields; website launch remains pending.
