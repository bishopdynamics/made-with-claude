...
Status: **portfolio v1 accepted by the user as ready for release**, 2026-09-08. The approved write-up, cover, and inline README GIF form v1. The entry is selected for public site builds; deployment remains a separate task. Additional gallery images belong to portfolio v2. These are portfolio-content versions, not changes to Continuum's software release tags.
...
The accepted article is at `src/content/projects/half-life-continuum-edition.md` with `draft: false`, confirmed `startedOn: '2026-06-09'`, release-based `completedOn: '2026-06-22'`, and source-backed language/tag metadata. `publishedOn: '2026-09-08'` records the prepared portfolio release date; reconcile it with the actual first-publication date during website launch if launch occurs later. The entry now appears in local homepage/catalog/public builds, while the domain launch is still queued.
...
| [Linked gameplay video](https://youtu.be/DVSHgFvknj0) | Public YouTube video “Half-Life: Continuum Edition - Unforeseen Consequences”. Embedded playback, controls, and fullscreen were verified in a browser on 2026-09-10; the site does not rehost it. | Embedded on the project page as `videoUrl` under `docs/spec/FEATURE_SPEC_project_video.md`, with `updatedOn: 2026-09-10`. |
...
- Selected completion: **2026-06-22**, using [v1.0.0.0](https://github.com/bishopdynamics/Continuum/releases/tag/v1.0.0.0) as the reproducible endpoint when finalizing the user's accepted v1. The orchestrator independently verified the release name, initial-release description, `published_at: 2026-06-22T19:10:26Z`, and Linux/macOS ARM64/Windows/Flatpak asset names. Later stabilization and maintenance do not automatically move this endpoint.
- The confirmed June 9 start through the June 22 initial release gives **13 calendar days**, not effort hours.
- Selected languages: **C, C++, Python, Shell** (`c`, `cpp`, `python`, `shell`). The independently inspected engine commit above modifies C sources; the [Continuum menu commit](https://github.com/bishopdynamics/mainui_cpp/commit/7d38e9ad13aa644108c525acb78a4e0da3ba2e8d), dated `2026-06-12T09:43:59Z`, adds C++ menu files. The initial umbrella commit adds `tools/hlstream_preprocess.py` and `tools/build-engine.sh`. C/C++ describe product implementation; Python/Shell describe project tooling. No language-byte percentages are used because the forks include substantial upstream code.
- Selected tags: Games, Graphics, Desktop (`games`, `graphics`, `desktop`). These and the language identifiers now exist in the shared taxonomy registry.
- Prepared portfolio release date: **2026-09-08**, the user's v1 acceptance date. Check the actual first-publication date during website launch; later maintenance remains distinct from the development endpoint.
...
- 2026-09-08: user accepted the assembled entry as v1 ready for release and chose BeamVM next. Completed source-backed metadata and public-build selection; no further Continuum v1 approval is pending.
- More gallery images are explicitly v2, mirrored in ROOT_SPEC and `docs/DEFERRED.md`. Do not hold v1 or BeamVM onboarding for those images.
...
- Website launch is the remaining external publication step, queued after BeamVM onboarding.
...
- Accepted-v1 verification: parent `make check` passed after actual public selection and metadata integration. The real build now contains four routes, one homepage tile and catalog entry for Continuum, the expected dates/languages, a 13-day development span, and no draft/noindex marker on its page. Log: `.agent-worktrees/_runs/root-slice-5/parent-release-check.log`. The older draft-exclusion result above describes the prior checkpoint, not the accepted v1 state.
