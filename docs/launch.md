...
Launched September 10, 2026 over HTTPS with Half-Life: Continuum Edition and FantasyBoy. BeamVM is paused for its code release; the six other entries remain drafts. Status here is updated as settings, deployment, and DNS are verified.
...
- Launch commit `4840dad` was pushed to main. [First launch run](https://github.com/bishopdynamics/made-with-claude/actions/runs/34519490584) failed seven safeguard tests; upload/deploy were skipped. The user supplied the full output in an ignored local scratch directory. Reproducing with a source path inside a runner-style home directory produced the same seven failures: the test fixture's directory-only `node_modules/` ignore rule accidentally staged its dependency symlink, whose target correctly triggered the privacy scanner. Fixed both fixture ignore rules and added ignored/unstaged assertions, preserving scanner policy. The same runner-style reproduction now passes all 15 safeguard tests; worker and parent full checks pass all 48 tests with zero Astro diagnostics and zero privacy findings across 300 snapshots. Parent independently reproduced the passing home-path setup. Fix commit `e2c71d5` passed both check and deploy jobs in [Actions run 34521004112](https://github.com/bishopdynamics/made-with-claude/actions/runs/34521004112).
- Apex DNS now resolves correctly on this machine as well as authoritative, Cloudflare, and Google resolvers. HTTP now serves the portfolio at `whatclaudemade.com`; www redirects to the apex. All 49 deployed files match the reviewed local artifact byte-for-byte, and all seven draft routes return 404. Live browser review confirms both articles, fonts, thumbnails, full-size FantasyBoy gallery, and the Continuum animation; mobile has no horizontal overflow. The initial HTTP verification was followed by the successful HTTPS checks below.

If certificate provisioning remains pending after DNS is correct, GitHub documents removing and re-saving the same custom domain to restart provisioning. Once the certificate is ready, enable Enforce HTTPS and verify apex/www HTTPS plus HTTP redirects. See [GitHub certificate provisioning troubleshooting](https://docs.github.com/en/pages/getting-started-with-github-pages/securing-your-github-pages-site-with-https#troubleshooting-certificate-provisioning-certificate-not-yet-created-error).

- **HTTPS launch verified September 10:** user removed/re-added the same custom domain and GitHub issued a Let’s Encrypt certificate covering `whatclaudemade.com` and `www.whatclaudemade.com`. Normal TLS hostname/trust validation passes. Apex HTTP redirects to apex HTTPS; www HTTPS redirects to apex HTTPS; www HTTP follows the apex HTTP redirect and then HTTPS. Both accepted articles, sitemap and robots return 200 over HTTPS; BeamVM stays 404. Browser navigation from www HTTP ends at canonical HTTPS in a secure context with decoded images, loaded fonts, no HTTP subresources, and no overflow. No local ACME client or renewal cron is required for GitHub-managed TLS. Evidence: `.agent-worktrees/_runs/site-launch/live-https-check.json`. The planned 15-minute wait was canceled when the user reported the certificate ready.
