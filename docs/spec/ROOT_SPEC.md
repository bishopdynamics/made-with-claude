...
6. **Launch workflow and final review — (L), [serial].** Add publication workflow, sitemap/robots, update authoring/launch instructions, run final site acceptance, then carry out the reviewed publication/domain steps when authorized.
   ...
   - **CI follow-up ownership:** `scripts/safeguards.test.mjs` for correcting the hosted runner's dependency-symlink fixture and verifying it stays out of scanned snapshots. Preserve production privacy policy and scanner behavior.
   - **Verification:** `make check`; parent full acceptance/browser review; inspect the exact artifact/workflow; reproduce hosted safeguard failures under a home-like source path before verifying their fix; after authorized publication, verify GitHub Actions, HTTPS/domain routing, and live site. No dummy content or unreviewed draft is published.
