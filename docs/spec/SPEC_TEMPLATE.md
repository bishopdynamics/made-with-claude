...
- **Scheduling:** `[serial]` (the default) or `[parallel-N]` — slices sharing a group number are farmed out to concurrent workers (see "Implementation approach" in `AGENTS.md`). Slices in the same parallel group must own **disjoint files**, and the shared contracts they build against (interfaces, types, stubs) must land in an earlier serial slice.
- **Owned files:** list them for **every** slice, serial or parallel, at spec-approval time — not lazily during implementation. They sharpen worker briefs, and they are the disjointness proof for parallel groups and for the cross-task parallelism check in `AGENTS.md`.

`[parallel-N]` means eligible for concurrent dispatch; it does not override checkout-isolation requirements. Parallel read-only workers may share a checkout. Give every parallel write worker a dedicated worktree and branch, or serialize the group when the agent platform cannot guarantee that isolation.
