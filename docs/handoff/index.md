...
- **2026-09-10 — GoatCounter visit counter implemented and verified locally; awaiting user review before publication.** After the Continuum video went live (see `archive_14.md`), the user asked for a site visit counter, chose hosted privacy-friendly analytics after two research rounds, created a GoatCounter account (site code `bishopdynamics`), briefly chose image-only, then reverted to the script version and approved `docs/spec/FEATURE_SPEC_visit_counter.md`. They asked whether the CDN matters for tracking; it does not, the beacon to the count endpoint is the tracking, and GoatCounter documents self-hosting `count.js`.
...
- **Next:** user reviews the footer sentence (may strike) and approves publication. Then push `main` to `github`, follow Actions, verify the live page loads the script and the beacon to `bishopdynamics.goatcounter.com/count` succeeds, user confirms the pageview in the dashboard, mark the TODO done, remove the task from the queue. Dashboard IP-ignore is the user's option for own visits. Docs (`development.md`, `launch.md`, `PROJECT.md`) already record the counter.
