...
1. Investigate the available skills, tools, and connectors — particularly `elefant` and `dev-tools` when available — so you know what is at your disposal.
...
- **Template-owned** — replaced verbatim by the template's migration tool; never edit them in a project: `AGENTS.md`, `CLAUDE.md`, `dev.sh`, `dispatch-codex.sh`, `.agent-hooks/`, `docs/spec/SPEC_TEMPLATE.md`, `docs/spec/TASK_BRIEF_TEMPLATE.md`, `docs/FIRST_RUN.md` (while it exists), the managed block at the top of `.gitignore`, and the safety hook registration plus `deny` entries in `.claude/settings.json` and `.codex/hooks.json`.
...
- **Models and reasoning:** pin each worker's exact model and reasoning effort when the platform supports it, so cost and capability are deliberate rather than accidental inheritance. These are role mappings, not claims that the vendors' models are behaviorally equivalent:
  - Claude Code: use Opus for demanding implementation and Sonnet for mechanical slices such as boilerplate, scaffolding, or rote refactors.
  - Codex: use `gpt-5.6-sol` with `high` reasoning for demanding implementation, `gpt-5.6-terra` with `medium` reasoning for lighter or read-heavy work, and `gpt-5.6-luna` with `low` reasoning only for narrow, repetitive, high-volume tasks.
  - Codex subagents inherit the parent model and reasoning effort when those values are omitted. Pass both at dispatch when possible; otherwise use a configured custom agent/default and record any intentional inheritance in the worker brief. Report every downgrade from the demanding-work tier as a medium choice.
  - **Claude Code orchestrating Codex workers:** a Claude Code session may hand a demanding implementation slice to Codex with the template-owned `./dispatch-codex.sh <slice> <brief-file>` (defaults `gpt-5.6-sol`, `high` reasoning, `workspace-write` sandbox; override with `-m`, `-e`, `-s`). It creates the worktree `.agent-worktrees/<slice>` on branch `codex/<slice>`, feeds the brief to `codex exec` on stdin, and writes `report.md`, `events.jsonl`, and `stderr.log` under `.agent-worktrees/_runs/<slice>/`. Run it through the shell tool in the background so several workers can run at once; the orchestrator is woken when it exits, then verifies and reviews the worktree exactly as for any other worker. Keep Claude's own Agent tool for Claude-model workers and read-only exploration. The reverse direction (Codex dispatching Claude) is not supported.
- **Isolation:** file ownership is not filesystem isolation. Parallel write workers use separate git worktrees and branches; the orchestrator integrates them in dependency order. Use built-in worktree isolation when available. Codex subagents may share the parent's filesystem by default, so the orchestrator must create each write worker's worktree, put its exact absolute working directory and branch in the brief, and ensure the worker operates there. If the dispatch interface cannot guarantee that working directory, keep workers read-only or run them serially. A lone serial worker may work in the main tree. Worktree isolation has a real per-worker cost in compiled/toolchain-heavy projects (cold build, per-checkout artifacts) — weigh that tax against the parallelism win when deciding serial vs parallel; small groups often lose. A shared build cache (e.g. `CARGO_TARGET_DIR`) is a mitigation with a cost: build-dir locking largely serializes concurrent builds, trading cold-build time against parallelism. Worktrees created by `dispatch-codex.sh` live under `.agent-worktrees/` (ignored via the managed `.gitignore` block); once integrated, remove them with `./dispatch-codex.sh --remove <slice>` and delete the branch.
- **Worktree lifecycle:** worktrees are temporary resources, not permanent project state. After a worker's changes are integrated or deliberately abandoned, stop the associated worker/session, verify the worktree is clean and its commits are merged or otherwise preserved, then remove the worktree without force and prune stale worktree metadata. Prefer the coding platform's safe session/worktree cleanup command when it manages both; otherwise use Git directly. Never remove a dirty, unmerged, locked, active, or uncertain worktree just to tidy the list. At session end, audit `git worktree list`; clean up completed worktrees and record the path, branch, owner, and reason for every intentional survivor in handoff.
...
- **Off-course workers:** course-correct through the platform's worker-messaging controls. If a worker breaks its brief (touched unowned files, can't pass verification), discard its isolated branch and re-dispatch with a better brief — don't hand-patch structural failures.
...
- Audit worker worktrees before handoff: remove safely completed worktrees, prune stale metadata, and document every intentional survivor with its branch, owner, and unresolved state.
